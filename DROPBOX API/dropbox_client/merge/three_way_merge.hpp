...
namespace MergeLib
{
   ...
   *
   * This function uses libgit2's git_merge_file API with tuned flags:
   *
   ...
   *       Ignores whitespace at the end of lines.
   *
   *   - GIT_MERGE_FILE_DIFF_PATIENCE:
   *       Uses the patience diff algorithm to reduce spurious diffs.
   *
   *   - GIT_MERGE_FILE_DIFF_MINIMAL:
   *       Takes extra time to find a minimal diff.
   *
   *   - GIT_MERGE_FILE_SIMPLIFY_ALNUM:
   *       Condenses non-alphanumeric regions to simplify diffing.
   *
   * Together, these flags aim to increase the success rate of automatic merges.
   */
  inline bool three_way_merge(const std::string &base, const std::string &local,
                              const std::string &remote, std::string &final)
  {
    // Initialize libgit2. In a larger application, do this once at startup.
    git_libgit2_init();
    ...
    // Set merge options with tuned flags.
