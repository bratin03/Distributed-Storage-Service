...
namespace kv
{
  ...
  RaftNode::RaftNode(uint64_t id, const std::string &cluster, uint16_t port, const std::string &ip, uint16_t redis_port, const std::string &redis_ip)
      : port_(port),
        ip_(ip),
  ...
  void RaftNode::schedule()
  {
    ...
    *conf_state_ = snap->metadata.conf_state;
    ...
    redis_server_ = std::make_shared<RedisStore>(this, std::move(snap_data_), port_, ip_, redis_port_, redis_ip_);
  ...
  void RaftNode::main(uint64_t id, const std::string &cluster, uint16_t port, const std::string &ip, uint16_t redis_port, const std::string &redis_ip)
  {
    ::signal(SIGINT, on_signal);
    ::signal(SIGHUP, on_signal);
    g_node = std::make_shared<RaftNode>(id, cluster, port, ip, redis_port, redis_ip);
