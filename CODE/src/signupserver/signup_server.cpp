...
inline void load_blockserver_config(const std::string &filepath) {
    ...
    MyLogger::info("Loaded blockserver config with " + std::to_string(blockserver_lists.size()) +
                   " server groups.");
}
...
bool signupUser(const std::string &username, const std::string &password,
                const std::string &redis_ip, int redis_port, std::string &errorMsg) {
    // Connect to Redis
    redisContext *context = redisConnect(redis_ip.c_str(), redis_port);
    if (context == nullptr || context->err) {
        errorMsg = "Redis connection error: " +
                   std::string(context ? context->errstr : "can't allocate redis context");
    ...
    redisReply *reply =
        (redisReply *)redisCommand(context, "SET %s %s NX", username.c_str(), password.c_str());
    if (!reply) {
...
int main(int argc, char *argv[]) {
    ...
    server.Post("/signup", [redis_ip, redis_port](const httplib::Request &req,
        ...
        try {
            ...
            if (!req_json.contains("username") || !req_json.contains("password")) {
                res.status = 400;
                res.set_content("{\"error\": \"Missing username or password\"}",
                                "application/json");
            ...
            std::string password =
                req_json["password"];  // In production, this should be a hashed password
