...
void initializeCache(std::chrono::milliseconds defaultTTL, std::size_t maxSize) {
    ...
    MyLogger::info("Cache initialized with default TTL: " + std::to_string(defaultTTL.count()) +
                   " ms and max size: " + std::to_string(maxSize) + " bytes");
}

json createFile(const std::string &file_key) {
    json payload = {{"path", file_key}};
    auto resp = login::makeRequest(login::metaLoadBalancerip, login::metaLoadBalancerPort,
                                   "/create-file", payload);
    if (resp == nullptr) {
...
bool uploadFile(const std::string &file_key) {
    ...
    auto version = fileMetadata._version;
    ...
    auto response = distributed_KV::setFile(endpoints, file_key, content, version, login::token,
                                            serverUtils::device_id);
    if (response.success) {
        MyLogger::info("File uploaded successfully to: " + file_key);
        // Update the metadata in the database
        fileMetadata._fileContent = content;
        // Version is incremented by 1 (it is string)
        fileMetadata._version = std::to_string(std::stoi(fileMetadata._version) + 1);
        fileMetadata._contentHash = fsUtils::computeSHA256Hash(content);
        fileMetadata._fileSize = content.size();
...
std::vector<std::string> getFileEndpoints(const std::string &file_key) {
    ...
    auto response = login::makeRequest(login::metaLoadBalancerip, login::metaLoadBalancerPort,
                                       "/get-file-endpoints", payload);
...
bool fetchNewFile(const std::string &file_key) {
    ...
    if (response.success) {
        ...
        if (response_json.contains("data") && response_json.contains("version_number")) {
            ...
            // Update the metadata in the database
            metadata::File_Metadata fileMetadata(file_key);
            fileMetadata._fileContent = file_content;
            fileMetadata._version = version_number;
            fileMetadata._contentHash = fsUtils::computeSHA256Hash(file_content);
            fileMetadata._fileSize = file_content.size();
            if (!fileMetadata.storeToDatabase()) {
                MyLogger::error("Failed to save file metadata to database for: " + file_key);
                return false;
            }
    ...
    if (response.success) {
        ...
        if (response_json.contains("data") && response_json.contains("version_number")) {
            ...
            if (local_file != remote_file_content) {
                ...
                auto merge_success = merge::mergeCheck(fileMetadata._fileContent, local_file,
                                                       remote_file_content, merged_content);
                if (merge_success) {
                    MyLogger::info("Merge successful for file: " + file_key);
                    fsUtils::createTextFile(file_key, merged_content);
                    fileMetadata._fileContent = merged_content;
                    fileMetadata._version = remote_version_number;
                    fileMetadata._contentHash = fsUtils::computeSHA256Hash(merged_content);
                    fileMetadata._fileSize = merged_content.size();
                    uploadFile(file_key);
                } else {
                    ...
                    // Update the metadata in the database
                    fileMetadata._fileContent = remote_file_content;
                    fileMetadata._version = remote_version_number;
                    fileMetadata._contentHash = fsUtils::computeSHA256Hash(remote_file_content);
                    fileMetadata._fileSize = remote_file_content.size();
                    if (!fileMetadata.storeToDatabase()) {
                        MyLogger::error("Failed to save file metadata to database for: " +
                                        file_key);
                    }
                    MyLogger::info("File metadata updated in database for: " + file_key);
                    // Send the conflicted copy to the server
                    metadata::File_Metadata conflictMetadata(conflict_filename);
                    conflictMetadata._fileContent = local_file;
                    conflictMetadata._version = "0";
                    conflictMetadata._contentHash = fsUtils::computeSHA256Hash(local_file);
                    conflictMetadata._fileSize = local_file.size();
                    metadata::addFileToDirectory(conflict_filename);
                    if (!conflictMetadata.storeToDatabase()) {
                        MyLogger::error("Failed to save conflict file metadata to database for: " +
                                        conflict_filename);
                    }
                    MyLogger::info("Conflict file metadata updated in database for: " +
                                   conflict_filename);
                    createFile(conflict_filename);
                    uploadFile(conflict_filename);
                    try {
                        AppNotify::send_notification(
                            "DSS: Conflict Detected",
                            "A conflict was detected for the file: " + file_key +
                                ". A conflicted copy has been created at: " + conflict_filename);
