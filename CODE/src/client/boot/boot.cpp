...
void syncDir(const fs::path &path) {
    ...
    dir_metadata._files = files;
    dir_metadata._directories = directories;
    ...
    auto prefix_scan =
        metadata::prefix_scan(directory_key + (directory_key.back() == '/' ? "" : "/"));
    for (const auto &file_key : prefix_scan) {
        MyLogger::debug("Checking file key: " + file_key);
        if (file_keys.find(file_key) == file_keys.end() &&
            dir_keys.find(file_key) == dir_keys.end()) {
...
void localToRemote() {
    ...
    if (dir_key.back() == '/') dir_key.pop_back();
...
void localToRemoteDirCheck(const std::string &dir_key) {
    ...
    for (const auto &file_key : dir_metadata._files) {
    ...
    for (const auto &subdir_key : dir_metadata._directories) {
...
void RemoteToLocalDirCheck(const std::string &dir_key) {
    ...
    for (const auto &dir_key : dir_metadata._directories) {
