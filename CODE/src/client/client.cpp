...
// Global constants
constexpr std::string_view user_info_config_path = "config/user_info.json";
constexpr std::string_view user_config_path = "config/user_config.json";
json user_info;
json user_config;
json server_info;
...
int main(int argc, char *argv[]) {
    ...
    login::handle_user_info(argc, argv, user_info, std::string(user_info_config_path));
    user_info = ConfigReader::load(std::string(user_config_path));
    ...
    auto status = rocksdb::DB::Open(options, metadata_path, &raw_db);
    if (!status.ok()) {
    ...
    std::thread notification_thread([&]() {
        process_remote::process_remote_events(notification_queue, queue_mutex, notification_cv,
                                              db_mutex);
    });

    notification::NotificationClient notification_client(
        serverUtils::notificationLoadBalancerip, serverUtils::notificationLoadBalancerPort,
        login::username, notification_queue, queue_mutex, notification_cv);
