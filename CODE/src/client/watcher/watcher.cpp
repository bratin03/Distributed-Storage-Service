...
void init_snapshot(const std::string &root_dir, std::unordered_set<std::string> &snapshot) {
...
std::vector<std::string> get_snapshot_entries(const std::unordered_set<std::string> &snapshot,
    ...
    for (const auto &p : snapshot) {
        if (is_descendant(base, p)) result.push_back(p);
    }
    return result;
}
...
void watch_directory(const std::string &root_dir, std::queue<FileEvent> &eventQueue,
    ...
    try {
        add_watch(root_dir);
        for (const auto &entry : fs::recursive_directory_iterator(root_dir)) {
            if (entry.is_directory()) add_watch(entry.path().string());
        }
        ...
        for (char *ptr = buffer; ptr < buffer + length;) {
...

            {
                ...
                if (event->mask & IN_CREATE) {
                    ...
                    if (event->mask & IN_ISDIR) add_watch(pathStr);
                ...
                // --- MOVED_FROM handling ---
                    ...
                    if ((event->mask & IN_ISDIR) && event->cookie != 0) {
                        ...
                        for (const auto &p : allEntries) snapshot.erase(p);
                        ...
                        for (const auto &child : children) {
                            FileEvent fevent{
                                InotifyEventType::MovedFrom, child,
                ...
                // --- MOVED_TO handling (using original logic) ---
                    ...
                    if ((event->mask & IN_ISDIR) && event->cookie != 0 &&
                        ...
                        {
                            FileEvent fevent{InotifyEventType::MovedTo, new_path,
                                             FileType::Directory};
                            if (eventMap.find(fevent) == eventMap.end()) {
                                eventQueue.push(fevent);
                                eventMap.insert(fevent);
                        ...
                        try {
                            for (const auto &entry : fs::recursive_directory_iterator(new_path)) {
                                ...
                                FileType ft =
                                    entry.is_directory() ? FileType::Directory : FileType::File;
