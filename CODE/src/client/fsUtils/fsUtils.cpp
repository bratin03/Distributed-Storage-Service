...
std::string buildKeyfromFullPath(fs::path fullPath) {
    ...
    if (key.rfind(g_basePath, 0) == 0) {
        ...
        MyLogger::error("Full path does not start with expected base path (" + g_basePath +
                        "): " + fullPath.string());
    }
    if (key.empty()) {
...
std::filesystem::path buildFullPath(const std::string &relativePath) {
    ...
    if (relativePath.rfind(prefix, 0) == 0) {
        ...
        MyLogger::error("Relative path does not start with expected prefix (" + prefix +
                        "): " + relativePath);
    }
...
void removeEntry(const std::string &relativePath) {
    ...
    if (std::filesystem::is_directory(entryPath, ec)) {
        ...
        if (ec)
            MyLogger::error("Failed to remove directory '" + entryPath.string() +
                            "': " + ec.message());
        else
            MyLogger::info("Removed directory (" + std::to_string(count) +
                           " entries): " + entryPath.string());
    } else {
