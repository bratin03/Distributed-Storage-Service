...
void process_local_events(std::queue<watcher::FileEvent> &eventQueue,
                          std::set<watcher::FileEvent> &eventMap, std::mutex &mtx,
    ...
    while (true) {
        ...
        cv.wait_for(lock, std::chrono::milliseconds(wait_time),
                    [&eventQueue]() { return eventQueue.size() >= wait_threshold; });

        // At this point, either the queue size threshold is reached (predicate_met is true)
        // or the timeout has expired (predicate_met is false). In either case, process the queued
        // events.
...
void file_modified(const std::string &path) {
    ...
    if (fileMetadata._contentHash == hash) {
