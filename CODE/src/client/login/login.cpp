...
void handle_server_info(json &server_info) {
    ...
    serverUtils::notificationLoadBalancerip =
        ConfigReader::get_config_string("notification_ip", server_info);
    serverUtils::notificationLoadBalancerPort =
        ConfigReader::get_config_short("notification_port", server_info);
...
void handle_user_info(int argc, char *argv[], json &user_info,
    ...
    if (argc == 1) {
        ...
        std::cout << "Usage: " << argv[0] << " [username] [password]" << std::endl;
        std::cout << "If no username and password are provided, the program will use the default "
                     "values from config/user_info.json"
                  << std::endl;
        exit(1);
    }
}
...
bool signup(const std::string &username, const std::string &password) {
    ...
    json payload;
    MyLogger::info("Sending signup request to " + signUpLoadBalancerip + ":" +
                   std::to_string(signUpLoadBalancerPort));
...
bool login() {
    ...
    MyLogger::info("Sending login request to " + loginLoadBalancerip + ":" +
                   std::to_string(loginLoadBalancerPort));
    ...
    if (res) {
        if (res->status == 200) {
            ...
            std::cout << "Login failed (" << std::to_string(res->status) << "): " << res->body
                      << std::endl;
            return false;
        }
    } else {
    ...
    if (token.empty()) {
        ...
        if (res) {
            if (res->status == 200) {
                ...
                MyLogger::error("Request failed (" + std::to_string(res->status) +
                                "): " + res->body);
                std::cout << "Request failed (" << std::to_string(res->status) << "): " << res->body
                          << std::endl;
