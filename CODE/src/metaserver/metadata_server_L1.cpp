...
void create_directory(const httplib::Request &req, httplib::Response &res) {
    ...
    try {
        ...
        const std::string key = userID + ":" + dir_id;
        ...
        if (last_slash == std::string::npos) {
            res.status = 400;
            MyLogger::warning(
                "Directory creation failed: Invalid path (no parent directory) for key: " + key);
        ...
        if (!parent_get_result.success) {
            res.status = 404;
            MyLogger::warning("Parent directory not found for key: " + parent_key +
                              " | Error: " + parent_get_result.err);
        ...
        try {
            ...
            if (parent_metadata.is_string()) {
                try {
                    ...
                    MyLogger::error("Failed to parse inner metadata for parent directory key: " +
                                    parent_key + " | Exception: " + std::string(e.what()));
                    res.status = 500;
                    res.set_content(R"({"error": "Failed to parse inner metadata"})",
                                    "application/json");
                    return;
                }
            }
            ...
            res.status = 500;
            MyLogger::error("Failed to parse parent directory metadata for key: " + parent_key +
                            " | Exception: " + std::string(e.what()));
            res.set_content(R"({"error": "Failed to parse parent directory metadata"})",
                            "application/json");
            return;
        }
        ...
        if (!set_result.success) {
            res.status = 500;
            MyLogger::error("Failed to write new directory metadata for key: " + key +
                            " | Error: " + set_result.err);
        ...
        if (!parent_key.empty())  // check required?????
        {
            ...
            if (!subdirs.is_array()) {
                res.status = 500;
                MyLogger::error("Parent metadata subdirectories is not an array for key: " +
                                parent_key);
            ...
            auto update_parent_result =
                Database_handler::set_directory_metadata(parent_key, parent_metadata);
            if (!update_parent_result.success) {
                MyLogger::warning(
                    "Directory created but failed to update parent metadata for key: " +
                    parent_key + " | Error: " + update_parent_result.err);
        ...
        json notification_payload = {
            {"type", "DIR+"}, {"user_id", userID}, {"path", dir_id}, {"device_id", device_id}};
...
void list_directory(const httplib::Request &req, httplib::Response &res) {
    ...
    try {
        ...
        try {
            ...
            res.status = 500;
            MyLogger::error("Failed to parse directory metadata for key: " + key +
                            " | Exception: " + std::string(e.what()));
...
void create_file(const httplib::Request &req, httplib::Response &res) {
    ...
    try {
        ...
        const std::string key = userID + ":" + file_path;
        ...
        if (last_slash == std::string::npos) {
            res.status = 400;
            MyLogger::warning(
                "File creation failed: Invalid file path (no parent directory) for key: " + key);
            res.set_content(R"({"error": "Invalid file path"})", "application/json");
            return;
        }
        ...
        if (!parent_res.success) {
            res.status = 404;
            MyLogger::warning("File creation failed: Parent directory not found for key: " +
                              parent_key + " | Error: " + parent_res.err);
            res.set_content(R"({"error": "Parent directory not found"})", "application/json");
            return;
        }
        ...
        auto parent_ok =
            Database_handler::set_directory_metadata(parent_key, parent_metadata.dump());
        if (!file_ok.success || !parent_ok.success) {
            res.status = 500;
            MyLogger::error("Failed to update KV store for file key: " + key +
                            " or parent key: " + parent_key);
            res.set_content(R"({"error": "Failed to store file metadata"})", "application/json");
            return;
        }

        res.status = 200;
        res.set_content(R"({"message": "File created", "metadata": )" + file_meta.dump() + "}",
                        "application/json");
        MyLogger::info("File created successfully for key: " + key);

        // Notify the notification server about the new file.
        json notification_payload = {
            {"type", "FILE+"}, {"user_id", userID}, {"path", file_path}, {"device_id", device_id}};
...
void delete_path(const httplib::Request &req, httplib::Response &res) {
    ...
    std::string userID;
    if (!Authentication::authenticate_request(req, res, userID)) return;
    ...
    bool is_file = path.size() >= 4 &&
                   path.substr(path.size() - 4) == ".txt";  // >=4 or >4 ?? doesnt really matter
    ...
    if (parent_meta_res.success) {
        ...
        if (parent_meta.is_string()) parent_meta = json::parse(parent_meta.get<std::string>());

        if (is_file && parent_meta["files"].is_array()) {
            auto &files = parent_meta["files"];
            files.erase(std::remove(files.begin(), files.end(), path),
                        files.end());                // remove the file from the parent directory
            DeletionManager::instance.enqueue(key);  // remove the data form the block storage

            // Notify the notification server about the new file.
            json notification_payload = {
                {"type", "FILE-"}, {"user_id", userID}, {"path", path}, {"device_id", device_id}};
            ...
            dirs.erase(std::remove(dirs.begin(), dirs.end(), path),
                       dirs.end());  // remove the directory from the parent directory
            // Notify the notification server about the new file.
            json notification_payload = {
                {"type", "DIR-"}, {"user_id", userID}, {"path", path}, {"device_id", device_id}};
        ...
        if (is_file) {
            MyLogger::warning("Parent directory not found for key: " + parent_key +
                              "proceeding to deleting the file anyways");
            DeletionManager::instance.enqueue(key);  // remove the data form the block storage
        }
    }
...
void get_file_endpoints(const httplib::Request &req, httplib::Response &res) {
    ...
    try {
        metadata = json::parse(kv_response.value);
        if (metadata.is_string()) metadata = json::parse(metadata.get<std::string>());
    } catch (const std::exception &e) {
        res.status = 500;
        MyLogger::error("Failed to parse file metadata for endpoint retrieval for key: " + key +
                        " | Exception: " + std::string(e.what()));
...
void block_server_confirmation(const httplib::Request &req, httplib::Response &res) {
    ...
    try {
        ...
        if (!body_json.contains("path") || !body_json.contains("user_id") ||
            !body_json.contains("device_id")) {
            res.status = 400;
            MyLogger::warning(
                "Block server confirmation failed: Missing 'path' or 'user_id' or 'device_id' in "
                "request body");
        ...
        std::string device_id = body_json["device_id"];
        MyLogger::debug("Processing block server confirmation for file: " + file_path +
                        " by user: " + userID);
...
int main(int argc, char *argv[]) {
    ...
    svr.set_logger([](const auto &req, [[maybe_unused]] const auto &res) {
        MyLogger::info("Request: " + req.method + " " + req.path);
    });
    ...
    */
    ...
    if (!svr.listen(Initiation::server_ip, Initiation::server_port)) {
        MyLogger::error(
            "Failed to start server. Check IP/port binding. Server IP: " + Initiation::server_ip +
            " | Server Port: " + std::to_string(Initiation::server_port));
