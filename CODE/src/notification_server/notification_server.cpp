...
NotificationServer::NotificationServer(asio::io_context &ioc, const std::string &ip,
                                       unsigned short port, int timeout_seconds)
...
void NotificationServer::do_accept() {
    acceptor_.async_accept([this](boost::system::error_code ec, tcp::socket socket) {
        if (!ec) {
            ...
            MyLogger::info("Client connected from IP: " + client_ip +
...
void NotificationServer::broadcastNotification(const std::string &user_id,
    ...
    try {
        ...
        MyLogger::error("Failed to parse JSON message for user: " + user_id +
                        ". Raw message: " + message);
    }
...
void Session::sendNotification(const std::string &message) {
    ...
    http::async_write(socket_, *res, [this, self, res](boost::system::error_code ec, std::size_t) {
        if (ec) {
