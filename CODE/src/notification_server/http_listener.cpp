...
#include "Mylogger.hpp"  // Include your custom logger.
   ...
   private:
    ...
    void handleRequest() {
        ...
        auto res = std::make_shared<http::response<http::string_body>>(
            http::response<http::string_body>{http::status::ok, req_.version()});
...
class HttpListener::Impl {
   public:
    Impl(asio::io_context &ioc, const std::string &ip, unsigned short port, RequestHandler handler)
        ...
        MyLogger::info("HttpListener initialized successfully on " + ip + ":" +
                       std::to_string(port));
    }
...
HttpListener::HttpListener(asio::io_context &ioc, const std::string &ip, unsigned short port,
