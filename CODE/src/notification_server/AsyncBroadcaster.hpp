...
class AsyncBroadcaster {
   public:
    ...
    explicit AsyncBroadcaster(const std::vector<Server> &servers) : servers_(servers) {}
    ...
    void sendBroadcastRequest(const Server &server, const nlohmann::json &message) {
        ...
        if (curl) {
            ...
            if (res != CURLE_OK) {
                std::cerr << "Broadcast request to " << url
                          << " failed: " << curl_easy_strerror(res) << std::endl;
            } else {
                std::cout << "Broadcast request to " << url << " completed successfully."
                          << std::endl;
            }
