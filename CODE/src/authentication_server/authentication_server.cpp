...
// Generate JWT token
std::string generateJWT(std::string_view userID) {
    MyLogger::info("Generating JWT token");

    auto token = jwt::create()
                     .set_issuer("auth-server")
                     .set_type("JWT")
                     .set_subject(std::string(userID))
...
json authenticateUser(const std::string &userID, const std::string &password) {
    ...
    auto *reply = (redisReply *)redisCommand(redis_ctx, "GET %s", userID.c_str());
    if (!reply) {
    ...
    svr.Post("/login", [](const httplib::Request &req, httplib::Response &res) {
        ...
        try {
            ...
            if (!body.contains("userID") || !body.contains("password")) {
                ...
                res.set_content(R"({"status": "error", "message": "Invalid JSON format"})",
                                "application/json");
                return;
            }
            ...
            res.status = 500;  // Internal Server Error
            res.set_content(R"({"status": "error", "message": "Server error"})",
                            "application/json");
        }
    });

    MyLogger::info("Authentication Server running on " + server_ip + ":" +
                   std::to_string(server_port) + "...");
    if (!svr.listen(server_ip, server_port)) {
        MyLogger::error("Failed to start server on " + server_ip + ":" +
                        std::to_string(server_port));
        return 1;
    }
