...
inline Response sendRequest(const std::string &url, const std::string &reqType,
...
inline Response redirectToLeader(const std::string &initialUrl, const std::string &reqType,
                                 const nlohmann::json &message) {
...
// Utility: Given a vector of server IP addresses, shuffle the vector and try each one until one
// returns a valid response.
inline Response tryServers(const std::vector<std::string> &servers, const std::string &reqType,
                           const nlohmann::json &message) {
 ...
 */
inline Response set(const std::vector<std::string> &servers, const std::string &key,
...
inline FileKVResponse setFile(const std::vector<std::string> &servers, const std::string &filePath,
                              const std::string &version) {
    FileKVResponse response;
    try {
        // Read file content
        std::string fileContent = readFileContent(filePath);
        // Build a JSON object with version number and file content.
        nlohmann::json j;
        j["version_number"] = version;
        j["data"] = fileContent;
        // Use filePath as the key
        Response setResp = set(servers, filePath, j.dump());
